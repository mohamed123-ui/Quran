...
export default function JuzsList() {
  ...
  const { juzs, loading, error } = useSelector(
    (state: RootState) => state.juzs
  );
  useEffect(() => {
    if (loading == "idle" || error == "failed" || juzs.length == 0) {
  ...
  return (
    <div className="my-8 p-5">
      <h1 className="text-center text-3xl font-bold mb-6 text-[#18A34B]">
        أجزاء القرآن الكريم
      </h1>
        ...
        {juzs.map((juz: JuzData) => (
          ...
          >
            <div className="flex justify-between items-center cursor-pointer">
              <h2 className="text-2xl font-semibold mb-2 text-[#18A34B]">
                الجزء رقم: {juz.number}
              </h2>
            ...
            {Object.values(juz.surahs).length > 0 && (
              <p className="text-gray-700 text-md mt-2">
                ...
                {Object.values(juz.surahs).map((surah) => (
                  <span
                    key={surah.number}
                    className="ml-1 font-medium flex gap-2"
                  >
                    ({surah.number})
                ...
                تبدأ بسورة:{" "}
                <span className="font-medium">
                  {Object.values(juz.surahs)[0].name}
                </span>
              </p>
            )}
          </div>
