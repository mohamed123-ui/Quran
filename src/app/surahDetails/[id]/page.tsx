...
export default function SurahDetailsPage() {
  ...
  const [selectedQari, setSelectedQari] = useState<string>(
    selectQari.identifier
  ); //مشاري
  useEffect(() => {
    if (id) {
      dispatch(surahDetails(Number(id))).then(() => {
        dispatch(
          surahAudio({ identifier: selectedQari, surahNumber: Number(id) })
        );
      });
    }
  }, [id, dispatch]);
  useEffect(() => {
    if (
      ...
      const alreadySaved = heard.find(
        (s) => s.surahNumber === currentSurah.number
      );
      if (!alreadySaved) {
        saveHeardSurah({
  ...
  return (
    ...
    >
      ...
      <p className="text-center text-gray-600">
        عدد الآيات: {currentSurah.numberOfAyahs} .{" "}
      ...
      <div>
        {currentSurah.fullAudio && (
          <div className="my-6 flex justify-center bg-[#F6F9FB] py-10 rounded-3xl px-6">
            <audio
              controls
              className="w-full md:w-2/3 bg-white rounded-lg shadow-md"
            >
        ...
        {currentSurah.ayahs.map((ayahs) => (
          <div key={ayahs.number} className="bg-white p-1 w-fit  rounded-md  ">
            <span className="block text-right w-full text-2xl font-[Amiri] leading-loose">
              {ayahs.text}
              <span className="bg-[#F3FAF6] p-2 rounded-4xl text-sm">
                {" "}
                {ayahs.numberInSurah}
              </span>
            </span>
          ...
          <Link href={`/surahDetails/${currentSurah.number - 1}`}>
            <p>
              <button
                ...
                className="border p-1 hover:text-[#51ba7f] disabled:cursor-no-drop cursor-pointer"
              >
                السوره السابقه
              </button>
            </p>
